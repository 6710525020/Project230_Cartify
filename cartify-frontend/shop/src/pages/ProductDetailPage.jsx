...
import { ArrowLeft, Package, Minus, Plus, Shield, Truck, RefreshCw } from 'lucide-react'
      ...
      <div className="gap-8 grid md:grid-cols-2">
        ...
        <div>
          <div className="bg-cream-200 shadow-card mb-3 rounded-2xl aspect-square overflow-hidden">
            {product.image || product.imageUrl
              ...
              : <div className="flex justify-center items-center w-full h-full text-brown-300 text-8xl">📦</div>
            }
          </div>
        ...
        <div className="flex flex-col gap-5">
          <div>
            ...
            <p className="font-display font-black text-brown-900 text-3xl">฿{product.price?.toLocaleString('th-TH')}</p>
          </div>
          ...
          {product.stock > 0 && user?.role === 'customer' && (
            <div className="flex items-center gap-3">
              ...
              <span className="font-body text-brown-400 text-xs">คงเหลือ: {product.stock} ชิ้น</span>
            </div>
          )}
          ...
          <div className="flex flex-col gap-2">
            ...
            {product.stock > 0 && (
              <Button variant="primary" size="lg" onClick={() => { handleAddCart(); setTimeout(() => navigate('/cart'), 300) }} disabled={product.stock === 0} className="rounded-xl w-full">
                ซื้อทันที
              </Button>
            )}
          </div>
          ...
          <div className="flex flex-wrap gap-2">
            {[
              ...
              { icon: RefreshCw, label: 'คืนสินค้าใน 7 วัน' },
            ].map((f, i) => (
