...
async function ensureCart(customerId) {
  let cart = await db.get2('SELECT cart_id FROM Cart WHERE customer_id = ?', [customerId]);
  if (!cart) {
    const { lastID } = await db.run2('INSERT INTO Cart (customer_id) VALUES (?)', [customerId]);
    cart = { cart_id: lastID };
  }
  return cart.cart_id;
}
...
async function addItem(req, res, next) {
  try {
    ...
    const product = await db.get2('SELECT product_id, stock FROM Product WHERE product_id = ?', [productId]);
    ...
    if (existing) {
      ...
      await db.run2('UPDATE CartItem SET quantity = ? WHERE cart_item_id = ?', [nextQty, existing.cart_item_id]);
    } else {
      await db.run2('INSERT INTO CartItem (cart_id, product_id, quantity) VALUES (?,?,?)', [cartId, productId, quantity]);
    }
...
async function updateItem(req, res, next) {
  try {
    ...
    await db.run2('UPDATE CartItem SET quantity = ? WHERE cart_item_id = ?', [quantity, itemId]);
...
async function removeItem(req, res, next) {
  try {
    ...
    await db.run2('DELETE FROM CartItem WHERE cart_item_id = ?', [itemId]);
    res.json(await loadCart(req.user.id));
  } catch (err) {
    next(err);
  }
...
async function clearCart(req, res, next) {
  try {
    const cartId = await ensureCart(req.user.id);
    await db.run2('DELETE FROM CartItem WHERE cart_id = ?', [cartId]);
