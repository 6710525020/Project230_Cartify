...
async function orderSummaryByStatus(req, res, next) {
  try {
    ...
    const grandTotal = await db.get2(`
      SELECT
        COUNT(*)                                  AS all_orders,
        ROUND(SUM(total_price)::numeric, 2)       AS all_revenue
      FROM "Order"
    `);
  ...
  try {
    const rows = await db.all2(`
      ...
      GROUP BY o.order_id, o.order_date, o.status, o.total_price,
               c.cname, c.email, c.phone_number
    ...
    const summary = await db.get2(`
      SELECT
        ...
        ROUND(SUM(o.total_price)::numeric, 2)       AS unpaid_total
      FROM "Order" o
      WHERE o.status NOT IN ('cancelled','completed')
...
async function revenueTrend(req, res, next) {
  try {
    const isMonthly = req.query.mode === 'monthly';
    const fmt   = isMonthly ? 'YYYY-MM' : 'YYYY-MM-DD';
    const label = isMonthly ? 'month' : 'date';
    const limit = parseInt(req.query.limit) || 30;

    const rows = await db.all2(`
      SELECT
        TO_CHAR(p.payment_date, '${fmt}')          AS ${label},
      ...
      GROUP BY TO_CHAR(p.payment_date, '${fmt}')
      ORDER BY 1 DESC
      LIMIT $1
    `, [limit]);
  ...
  try {
    const rows = await db.all2(`
      SELECT
        ...
        ROUND(SUM(CASE WHEN o.status = 'completed' THEN o.total_price ELSE 0 END)::numeric, 2)
                                                                                   AS revenue_managed,
        ROUND(
...
async function searchOrders(req, res, next) {
  try {
    ...
    if (from_date)   { conditions.push(`o.order_date >= $${idx++}`);   params.push(from_date); }
    if (to_date)     { conditions.push(`o.order_date <= $${idx++}`);   params.push(to_date); }
